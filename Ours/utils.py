...
def process_data():
    ...
    node_temp = pd.concat([cite_edges.loc[:, 'source'], cite_edges.loc[:, 'target'], ref_edges.loc[:, 'target']])
    node_papers = pd.DataFrame(index=pd.unique(node_temp))

    node_temp = pd.concat([ref_edges['source'], coauthor_edges['source'], coauthor_edges['target']])
    node_authors = pd.DataFrame(index=pd.unique(node_temp))
...
def train_metapath(g, path, ntype):
    ...
    embedding = model.node_embed(nids)
    return embedding
