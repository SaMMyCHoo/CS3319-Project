...
def train(args, hetero_graph, test_refs, rel_list, device):
    ...
    sampler = dgl.dataloading.as_edge_prediction_sampler(
        sampler, negative_sampler=dgl.dataloading.negative_sampler.PerSourceUniform(args.k))
    ...
    for epoch in range(args.num_epochs):
        ...
        label_pred = np.array(res)
        median = np.median(label_pred)

        cur_best_f1_score = 0
        for i in range (-50, 50):
            threshold = best_thresh + 0.001 * i if best_thresh is not None else median + 0.001 * i
            label_pred_copy = label_pred.copy()
            label_pred_copy[label_pred >= threshold] = 1
            label_pred_copy[label_pred < threshold] = 0
            # Test scores
            precision, recall, f1_score, _ = precision_recall_fscore_support(label_true, label_pred_copy, average='binary')
            if f1_score > cur_best_f1_score:
                cur_pr = precision
                cur_re = recall
                cur_best_f1_score = f1_score
                cur_best_thresh = threshold
                acc = (label_true == label_pred_copy).mean()
        if cur_best_f1_score > best_f1_score:
            print('NEW BEST PREDICTION!')
            best_f1_score = cur_best_f1_score
            best_embed = node_embeddings
            best_thresh = cur_best_thresh
        
        t2 = time.time()
        print(f"acc:{acc:.4f}, precision:{cur_pr:.4f}, recall:{cur_re:.4f}, f1_score-Score: {cur_best_f1_score:.5f}, best: {best_f1_score:.5f}, time: {(t2-t1):.2f}s")

    return best_embed, best_thresh, best_f1_score
